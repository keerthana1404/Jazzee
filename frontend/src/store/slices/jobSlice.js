...
export const fetchJobs =
  ...
  async (dispatch) => {
    try {
      ...
      let link = "https://jazzee-7.onrender.com/api/v1/job/getall?";
      let queryParams = [];
      if (searchKeyword) {
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://jazzee-7.onrender.com/api/v1/job/get/${jobId}`,
      { withCredentials: true }
    );
    // console.log(response.data);
    
    dispatch(jobSlice.actions.successForSingleJob(response.data.thatJob));
    dispatch(jobSlice.actions.clearAllErrors());
  } catch (error) {
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `https://jazzee-7.onrender.com/api/v1/job/post`,
      data,
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://jazzee-7.onrender.com/api/v1/job/getmyjobs`,
      { withCredentials: true }
    );
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://jazzee-7.onrender.com/api/v1/job/delete/${id}`,
      { withCredentials: true }
    );
